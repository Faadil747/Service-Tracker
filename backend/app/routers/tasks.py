...
async def approve_task(
    ...
    if approved and req.publish_directly:
        # Admin approves the agent's draft AND pushes it live in one step — no
        # round-trip back to the agent to publish. This is only valid when there
        # is an unpublished draft attached to review.
        from app.models.post import PostStatus
        if not post:
            raise HTTPException(
                status_code=400,
                detail="There is no draft post attached to this task to publish. Use plain Approve instead.",
            )
        if post.status == PostStatus.published:
            raise HTTPException(status_code=400, detail="This post has already been published.")
        ...
        # Reuse the single source of truth for going live: publishes to LinkedIn,
        # marks the post published, completes this task (+ notifies admins) and
        # creates the UTM tracking link. Keeps the direct-publish path byte-for-byte
        # identical to the agent's own publish path — no divergence, no dead code.
        from app.routers.posts import publish_post_to_linkedin
